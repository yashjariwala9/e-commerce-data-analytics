...
PRODUCT_BRAND_MAPPING = {
    'Electronics': {
        'brands': ['Samsung', 'Apple', 'OnePlus', 'Xiaomi', 'Realme', 'Oppo', 'Vivo', 'Sony', 'LG', 'Panasonic', 'Boat', 'JBL', 'Dell', 'HP', 'Lenovo', 'Asus'],
        'subcategories': ['Smartphones', 'Laptops', 'Tablets', 'Headphones', 'Cameras', 'TVs', 'Audio Systems', 'Smart Watches', 'Gaming Consoles', 'Accessories']
    },
    'Clothing & Fashion': {
        'brands': ['Zara', 'H&M', 'Uniqlo', 'Fabindia', 'W for Woman', 'Biba', 'Aurelia', 'Peter England', 'Van Heusen', 'Allen Solly', "Levi's", 'Pepe Jeans', 'Manyavar', 'Sabyasachi'],
        'subcategories': ['Kurtas', 'Sarees', 'Jeans', 'T-Shirts', 'Formal Shirts', 'Dresses', 'Footwear', 'Accessories', 'Ethnic Wear', 'Western Wear']
    },
    'Home & Furniture': {
        'brands': ['IKEA', 'Godrej', 'Nilkamal', 'Durian', 'Urban Ladder', 'Pepperfry', 'HomeTown', 'Sleepwell', 'Kurlon', 'Wipro Furniture', 'Spacewood'],
        'subcategories': ['Sofas', 'Beds', 'Dining Tables', 'Wardrobes', 'Kitchen Appliances', 'Home Decor', 'Mattresses', 'Lighting', 'Storage Solutions', 'Garden Furniture']
    },
    'Beauty & Personal Care': {
        'brands': ['Lakme', 'Maybelline', "L'Oreal", 'Nykaa', 'Mamaearth', 'Himalaya', 'Dabur', 'Patanjali', 'Nivea', 'Dove', 'Forest Essentials', 'Biotique'],
        'subcategories': ['Skincare', 'Makeup', 'Hair Care', 'Fragrances', "Men's Grooming", 'Ayurvedic Products', 'Body Care', 'Nail Care', 'Tools & Accessories']
    },
    'Food & Beverages': {
        'brands': ['Amul', 'Britannia', 'Parle', 'ITC', 'Nestle', 'Coca-Cola', 'PepsiCo', "Haldiram's", 'MTR', 'Maggi', 'Tata Tea', 'Mother Dairy', 'Everest'],
        'subcategories': ['Snacks', 'Beverages', 'Dairy Products', 'Ready-to-Eat', 'Spices & Condiments', 'Tea & Coffee', 'Sweets & Desserts', 'Health Foods', 'Organic Products']
    },
    'Books & Stationery': {
        'brands': ['Penguin', 'Oxford', 'Cambridge', 'Classmate', 'Reynolds', 'Camlin', 'Faber-Castell', 'Navneet', 'Apsara', 'Cello'],
        'subcategories': ['Fiction', 'Non-Fiction', 'Educational', "Children's Books", 'Stationery', 'Art Supplies', 'Notebooks', 'Writing Instruments', 'Academic Books']
    },
    'Sports & Fitness': {
        'brands': ['Adidas', 'Nike', 'Puma', 'Reebok', 'Decathlon', 'Nivia', 'Cosco', 'Yonex', 'Li-Ning', 'Victor', 'Spalding'],
        'subcategories': ['Cricket Equipment', 'Fitness Equipment', 'Athletic Wear', 'Outdoor Sports', 'Yoga & Wellness', 'Badminton', 'Football', 'Basketball', 'Swimming']
    },
    'Automotive': {
        'brands': ['Maruti Suzuki', 'Hyundai', 'Tata Motors', 'Mahindra', 'Hero', 'Bajaj', 'TVS', 'Bosch', 'MRF', 'Apollo Tyres', 'Castrol'],
        'subcategories': ['Car Accessories', 'Bike Parts', 'Tyres', 'Engine Oil', 'Car Care Products', 'Interior Accessories', 'Safety Equipment', 'Performance Parts']
    },
    'Health & Wellness': {
        'brands': ['Dabur', 'Himalaya', 'Patanjali', 'Zandu', 'Revital', 'Centrum', 'Protinex', 'Complan', 'Horlicks', 'Baidyanath'],
        'subcategories': ['Ayurvedic Medicine', 'Vitamins', 'Supplements', 'First Aid', 'Personal Hygiene', 'Health Drinks', 'Herbal Products', 'Medical Devices']
    },
    'Toys & Games': {
        'brands': ['Funskool', 'Mattel', 'Hasbro', 'Lego', 'Fisher-Price', 'Hot Wheels', 'Barbie', 'Nerf', 'Play-Doh'],
        'subcategories': ['Educational Toys', 'Action Figures', 'Board Games', 'Dolls', 'Electronic Toys', 'Building Blocks', 'Puzzles', 'Outdoor Toys', 'Soft Toys']
    }
}
...
class UnifiedDataGenerator:
    ...
    @staticmethod
    def clamp_value(value, min_value=0):
    ...
    def generate_stores(self, num_stores=50):
        ...
        for i in range(num_stores):
            ...
            store = {
                ...
                'InventoryCapacity': random.randint(1000, 20000) if store_type != 'Online Only' else 0
            }
            stores.append(store)
    ...
    def generate_machines(self, num_machines=300):
        ...
        if 'employees' in self.data and not self.data['employees'].empty:
            eligible_employee_ids = self.data['employees'][self.data['employees']['Role'].isin(manufacturing_roles)]['EmployeeID'].tolist()
    ...
    def generate_orders_details(self, num_orders=None):
        ...
        qty_ranges = {'Food & Beverages': (1, 10), 'Beauty & Personal Care': (1, 10), 'Clothing & Fashion': (1, 5), 'Books & Stationery': (1, 5), 'Toys & Games': (1, 5), 'Electronics': (1, 2), 'Home & Furniture': (1, 2), 'Sports & Fitness': (1, 2), 'Automotive': (1, 2), 'Health & Wellness': (1, 2)}
        discount_ranges = {'Electronics': (0, 0.2), 'Clothing & Fashion': (0.1, 0.5), 'Food & Beverages': (0, 0.15), 'Beauty & Personal Care': (0.05, 0.3), 'Home & Furniture': (0.05, 0.25), 'Books & Stationery': (0, 0.2), 'Sports & Fitness': (0, 0.2), 'Automotive': (0, 0.2), 'Health & Wellness': (0, 0.2), 'Toys & Games': (0, 0.2)}
        gst_rates = {'Food & Beverages': 0.05, 'Books & Stationery': 0.05, 'Clothing & Fashion': 0.12, 'Beauty & Personal Care': 0.12, 'Electronics': 0.18, 'Home & Furniture': 0.18, 'Sports & Fitness': 0.18, 'Automotive': 0.18, 'Health & Wellness': 0.18, 'Toys & Games': 0.18}
        ...
        product_dict = self.data['products'].set_index('SKU').to_dict('index')
        ...
        for i in range(num_orders):
            ...
            status = random.choices(['Pending', 'Processing', 'Shipped', 'Delivered', 'Cancelled', 'Exchanged', 'Refunded'],
                                    weights=[0.3, 0.3, 0.25, 0.1, 0.02, 0.02, 0.03])[0]
            ...
            for _ in range(num_items):
                ...
                prod_info = product_dict[sku]
                category = prod_info['Category']
                cost_price = float(prod_info['CostPrice'])
    ...
    def generate_inventory_levels(self):
        ...
        wh_dict = self.data['warehouses'].set_index('WarehouseID').to_dict('index')
        ...
        inventory_ranges = {
            ...
            'Automotive': (50, 1500), 'Health & Wellness': (50, 1500), 'Toys & Games': (50, 1500),
            'Home & Furniture': (50, 1500)
        }
        for _, product in self.data['products'].iterrows():
            selected_warehouses = random.sample(wh_ids, random.randint(1, min(3, len(wh_ids))))
            for warehouse_id in selected_warehouses:
                warehouse = wh_dict[warehouse_id]
    ...
    def generate_production_runs(self, num_runs=5000):
        ...
        production_ranges = {
            ...
            'Health & Wellness': (50, 500), 'Toys & Games': (50, 500), 'Home & Furniture': (50, 500),
            'Electronics': (50, 500)
        }
        
        machine_ids = self.data['machines']['MachineID'].tolist()
        skus = self.data['products']['SKU'].tolist()
        manufacturing_employees = self.data['employees'][self.data['employees']['Department'] == 'Manufacturing']['EmployeeID'].tolist() if 'employees' in self.data else []
        sku_to_cat = dict(zip(self.data['products']['SKU'], self.data['products']['Category']))
    ...
    def generate_shipments(self, num_shipments=8000):
        ...
        if 'employees' in self.data and not self.data['employees'].empty:
            eligible_employee_ids = self.data['employees'][self.data['employees']['Role'].isin(eligible_roles)]['EmployeeID'].tolist()
        else:
            eligible_employee_ids = []
        ...
        warehouse_ids = self.data['warehouses']['WarehouseID'].tolist()
        order_to_customer = dict(zip(self.data['orders_details']['OrderID'], self.data['orders_details']['CustomerID']))
        cust_to_loc = dict(zip(self.data['customers']['CustomerID'], self.data['customers']['Location']))
        wh_to_loc = dict(zip(self.data['warehouses']['WarehouseID'], self.data['warehouses']['Location']))

        for i in range(num_shipments):
            selected_order = unique_orders.sample(1).iloc[0]
            order_id = selected_order['OrderID']
            order_date = selected_order['OrderDate']
            ...
            warehouse_city = wh_to_loc[warehouse_id]
            customer_city = cust_to_loc[customer_id]
            ...
            initial_delivery_date = order_date + timedelta(days=delivery_days)
            
            delay_chance = random.random()
            if delay_chance > 0.7:
                additional_delay_days = random.randint(1, 3)
                delivery_date = initial_delivery_date + timedelta(days=additional_delay_days)
                ...
                if delivery_date < today:
                    status_options = ['Delivered', 'Returned', 'Failed']
            ...
            if delivery_date > today and status not in ['In Transit', 'Out for Delivery', 'Pending']:
                delivery_date = today
    ...
    def generate_routes(self):
        ...
        emp_to_veh = dict(zip(self.data['employees']['EmployeeID'], self.data['employees']['VehicleType']))
        order_to_customer = dict(zip(self.data['orders_details']['OrderID'], self.data['orders_details']['CustomerID'])) if 'orders_details' in self.data else {}
        cust_to_loc = dict(zip(self.data['customers']['CustomerID'], self.data['customers']['Location']))
        wh_to_loc = dict(zip(self.data['warehouses']['WarehouseID'], self.data['warehouses']['Location']))
        all_cust_ids = self.data['customers']['CustomerID'].tolist()
        ...
        for i, shipment in enumerate(self.data['shipments'].iterrows()):
            ...
            if employee_id in driver_employee_ids:
                vehicle_type = emp_to_veh.get(employee_id, random.choice(VEHICLE_TYPES))
            else:
            ...
            traffic_factor = {
                'Low': random.uniform(1.0, 1.3),
                'Medium': random.uniform(1.3, 1.7),
                'High': random.uniform(1.7, 2.0),
                'Very High': random.uniform(2.0, 2.5)
            }
            ...
            if pd.isna(customer_id) or customer_id is None:
                customer_id = random.choice(all_cust_ids)

            end_location = cust_to_loc.get(customer_id, random.choice(WAREHOUSE_LOCATIONS))
            start_location = wh_to_loc.get(shipment_data['WarehouseID'])

            ship_date = shipment_data['ShipDate']
            if not pd.api.types.is_datetime64_any_dtype(ship_date):
                ship_date = pd.to_datetime(ship_date)
                
            start_hour = random.randint(8, 18)
            start_time = pd.Timestamp(ship_date) + pd.Timedelta(hours=start_hour, minutes=random.randint(0, 59))
    ...
    def generate_employees(self, num_employees=250):
        ...
        if 'employees' in self.data and not self.data['employees'].empty:
            existing_employees = self.data['employees'].to_dict('records')
        else:
            existing_employees = []
        ...
        eligible_roles = ['Driver', 'Warehouse Manager', 'Logistics Coordinator']
        if 'shipments' in self.data and not self.data['shipments'].empty:
            eligible_shipment_ids = self.data['shipments']['ShipmentID'].tolist()
        else:
            eligible_shipment_ids = []
    ...
    def generate_initial_employees(self, num_drivers=50):
        ...
        availability_options = {
            'Driver': ['Available', 'On Trip', 'Off Duty', 'On Break']
        }
